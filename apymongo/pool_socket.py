...
from apymongo.errors import ConnectionFailure, OperationFailure
...
class BasePool(object):
    ...
    def create_connection(self, pair, callback):
            ...
            try:
                ...
                if self.use_ssl:
                    try:
                        ...
                        sock.close()
                        callback(ConnectionFailure("SSL handshake failed. MongoDB may "
        ...
        if err is not None:
            ...
            # support IPv6 at all.
            callback(socket.error('getaddrinfo failed'))
    ...
    def connect(self, pair, callback):
        ...
        """
        sock = self.create_connection(pair, callback)
