...
from apymongo.errors import DuplicateKeyError
...
class GridOut(object):
    ...
    """
    @classmethod
    def instance_for(cls, root_collection, callback, file_id=None, file_document=None):
        instance = GridOut(root_collection, file_id, file_document)
        files = root_collection.files
        def mod_callback(response):
            """docstring for mod_callback"""
            instance._file = response
            
            if not instance._file:
    ...
    def read(self, callback, size=-1):
        ...
        while received < size:
            chunk = yield gen.Task(self.__chunks.find_one, {"files_id": self._id,
                                            "n": chunk_number})
            # chunk = self.__chunks.find_one({"files_id": self._id,
                                            # "n": chunk_number})
            if not chunk:
                callback(CorruptGridFile("no chunk #%d" % chunk_number))
                return
            if received:
    ...
    @gen.engine
    def readline(self, callback, size=-1):
        ...
        while len(bytes) != size:
            byte = yield gen.Task(self.read, 1)
            bytes += byte
...
class GridOutIterator(object):
    ...
    def next(self):
        if self.__current_chunk >= self.__max_chunk:
            raise StopIteration
        chunk = yield gen.Task(self.__chunks.find_one, {"files_id": self.__id, "n": self.__current_chunk})
        if not chunk:
            raise CorruptGridFile("no chunk #%d" % self.__current_chunk)
        self.__current_chunk += 1
        callback(binary_type(chunk["data"]))
